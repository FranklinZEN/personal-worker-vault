...
class HashChainedLedger:
    ...
    def append(self, candidate: dict[str, Any]) -> dict[str, Any]:
        ...
        preliminary = self.schemas.issues(self.schema_name, candidate)
        essential_paths = {
            f"$/{self.id_field}",
        ...
        try:
            with _exclusive_lock(lock_path):
                ...
                candidate_issues = list(self.schemas.issues(self.schema_name, final))
                try:
            ...
            if committed and stage_path.exists():
                stage_path.unlink()
    ...
    def scan(self, path: Path) -> ScanResult:
        ...
        for line_number, framed in enumerate(data.splitlines(keepends=True), start=1):
            ...
            if not is_canonical:
                ...
                try:
                    self.schemas.require(self.schema_name, record)
                except ValidationError as exc:
                    issues.extend(exc.issues)
    ...
    def recover(
        ...
        *,
        ...
        with _exclusive_lock(lock_path):
            ...
            decision = policy.evaluate(fresh, approvals=approvals, now=now)
...
class SemanticLedger(HashChainedLedger):
    ...
    def domain_issues(
        ...
        for issue in self.schemas.issues(payload_schema, record["payload"]):
        ...
        if event_type != "case.created" and record["case_id"] not in case_ids:
            issues.append(
